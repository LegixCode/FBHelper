...
import { getAccountStatusText, getDisableReasonText, IAdaccount } from "@/classes/FB/types/IAdaccount";
...
export async function getAdaccounts(accessToken: string): Promise<IAdaccount[]> {
    ...
    do {
        ...
        if (response.error?.message) {
            if (response.error.message.startsWith("Please reduce the amount") && parseInt(params.limit) > 10) {
                params.limit = (parseInt(params.limit) - 5).toString();
                after = null;
                continue;
            }
