...
import { execContentScriptAction } from "@/utils/chromeMethods";
import { getAdaccounts } from "./requests/getAdaccounts";
import { getBillingHoldStatus } from "./requests/getBillingHoldStatus";
import { getCards } from "./requests/getCards";
import { getUserIdFromCookies } from "@/classes/cookies";
import { parseAccessToken } from "./parseAccessToken";

export type TAdaccountSortType = "name" | "status";

export class FBStore {
    static self: FBStore = null;
    ...
    readonly userId = computed(() => {
        if (!this._userId.value) {
            getUserIdFromCookies((value) => (this._userId.value = value));
        }
